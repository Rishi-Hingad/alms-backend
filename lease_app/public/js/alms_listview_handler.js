frappe.provide('lease_app');

lease_app.restricted_modules = ['ALMS', 'CRMS', 'master'];

lease_app.restrict_listview = function (listview) {
    const doctype = listview.doctype;

    frappe.call({
        method: 'lease_app.utils.utils.get_doctype_module',
        args: { doctype: doctype },
        async: false,
        callback: function (r) {
            if (r.message && lease_app.restricted_modules.includes(r.message)) {
                frappe.call({
                    method: 'lease_app.utils.utils.can_view_alms_doctypes',
                    async: false,
...
frappe.listview_settings = new Proxy(frappe.listview_settings || {}, {
    get: function (target, doctype) {
        ...
        target[doctype].onload = function (listview) {
            lease_app.restrict_listview(listview);
