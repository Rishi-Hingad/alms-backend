...
from lease_app.api.invoice_details import _link_invoice_to_lease, map_invoice_row
...
def _upsert_invoice_batch(batch):
	"""
	Upsert a single batch into LMS Invoice Details.
	"""
	...
	if not existing_name:
		doc = frappe.new_doc("LMS Invoice Details")
		...
		if excel_file_url:
			_relink_file_to_doc(excel_file_url, "LMS Invoice Details", doc.name)
		if invoice_attach_url:
			_relink_file_to_doc(invoice_attach_url, "LMS Invoice Details", doc.name)
	...
	# ------------------------------------------------------------------
	doc = frappe.get_doc("LMS Invoice Details", existing_name)
