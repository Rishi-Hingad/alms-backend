frappe.ui.form.on("Vehicle Details", {
    car_allotment_letter: function(frm) {
        if (frm.doc.car_allotment_letter) {
            frappe.call({
                method: "lease_app.crms.doctype.vehicle_details.vehicle_details.send_car_allotment_email",
                args: {
    ...
    rc_book: function(frm) {
        if (frm.doc.rc_book) {
            frappe.call({
                method: "lease_app.crms.doctype.vehicle_details.vehicle_details.send_rc_book_email",
                args: {
                    docname: frm.doc.name,
                    employee_code: frm.doc.employee_code_and_name,
