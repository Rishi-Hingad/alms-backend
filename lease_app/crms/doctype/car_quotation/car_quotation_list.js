                    ...
                    [
                        {
                            ...
                            get_query: () => ({
                                query: "lease_app.crms.doctype.car_quotation.car_quotation.get_available_purchase_forms"
                            }),
                            reqd: 1
                        }
                    ],
...
function openQuotationDialog(data, role, purchaseForm, listview) {
    ...
    d.$wrapper.on("click", ".approve, .reject", async function () {
        ...
        try {
            ...
            await frappe.call({
                method: "lease_app.crms.doctype.car_quotation.car_quotation.process_workflow",
                args: {
                    quotation_id: id,
                    action: action,
                    remarks: remarks
    ...
    d.$wrapper.on("click", ".send-revised", async function () {
        ...
        const res = await frappe.call({
            method: "lease_app.crms.doctype.car_quotation.car_quotation.create_revised_quotation",
            args: { old_id: id }
        });
