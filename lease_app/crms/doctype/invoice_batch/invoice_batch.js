...
frappe.ui.form.on("Invoice Batch", {
    ...
    refresh(frm) {
        ...
        if (frm.doc.excel_file) {
            if (frm.doc.excel_sheet_status === "Pending" || frm.doc.excel_sheet_status === "Failed") {
                ...
                frm.add_custom_button("Retry Failed Rows", () => {
                    frappe.call({
                        method: "lease_app.crms.doctype.invoice_batch.invoice_batch.retry_failed",
                        args: {
                ...
                frm.add_custom_button("Download Error Report", () => {
                    window.open(
                        `/api/method/lease_app.crms.doctype.invoice_batch.invoice_batch.download_error_report?docname=${frm.doc.name}`
                    )
                })
            }
        }
        ...
        if (
            ...
            frm.add_custom_button("Retry API", () => {
                ...
                frappe.call({
                    method: "lease_app.crms.doctype.invoice_batch.invoice_batch.retry_lease_api",
                    args: {
                        docname: frm.doc.name
                    },
                    freeze: true,
    ...
    render_pdf_preview: function (frm, attachment_field, preview_field) {
        ...
        frappe.call({
            method: "lease_app.crms.doctype.invoice_batch.invoice_batch.get_file_preview_url",
            args: {
                file_url: file_url
            },
            callback: function (r) {
