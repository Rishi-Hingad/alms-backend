...
frappe.ui.form.on("Company and Employee Deduction", {
    ...
    refresh(frm) {
        ...
        if (!frm.is_new() && frm.doc.status === "Approved") {
            frm.add_custom_button(__("Send Email to Employee"), () => {
                frappe.call({
                    method: "lease_app.crms.doctype.company_and_employee_deduction.company_and_employee_deduction.send_employee_approval_email",
                    args: {
