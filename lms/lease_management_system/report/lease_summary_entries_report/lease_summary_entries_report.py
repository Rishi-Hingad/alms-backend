...
def execute(filters=None):
	...
	if not frappe.db.exists("GL Code Master", {"company": company_name}):
		frappe.throw(
			f"GL Code doesn't exist for the selected company: {company_name}"
		)

	gl_code_doc= frappe.get_doc("GL Code Master",company_name)

	result = run("Lease Summary Report", filters={"company_name": company_name,"fin_start_year":fin_start_year,"fin_end_year":fin_end_year})
	...
	columns=[
		...
		{"label": "Debit", "fieldname": "debit", "fieldtype": "Currency", "width": 200,"precision": 2,},
		{"label": "Credit", "fieldname": "credit", "fieldtype": "Currency", "width": 200,"precision": 2,},
	]
	records=[
		{
