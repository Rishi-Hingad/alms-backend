...
def get_employee_details(employee_code):
    employee = frappe.get_all("Employee", filters={"name": employee_code}, 
    ...
    if not employee:
        ...
        employee = frappe.get_all("Employee", filters={"employee_code": possible_code}, 
        fields=["name", "designation", "location", "company", "contact_number", "email_id", "department","eligibility","reporting_head"])
...
def send_email_to_reporting_head(car_indent_form_name):
    ...
    try:
        ...
        employee = frappe.get_doc("Employee", car_indent.employee_code)
