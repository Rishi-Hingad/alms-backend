...
# @frappe.whitelist()
# def process_eazy_assets(file_url):
...
#     # Create Invoice Batch only if errors exist
#     if failed_rows:
...
#     if failed_rows:
#         msg += f"<br>{len(failed_rows)} rows moved to Invoice Batch: <b>{batch.name}</b>"
...
def process_eazy_assets(file_url, vendor=None):
    ...
    for idx, row in enumerate(data, start=1):
        ...
        try:
            ...
            batch.append("failed_rows_table", {
                "row_number": idx,
                "contract_number": mapped.get("contract_number"),
                "installment_no": mapped.get("installment_no"),
                "error_message": str(e)
    ...
    batch.total_value_of_company_contribution = 0
    batch.total_value_of_employee_contribution = 0
...
def process_ald(file_url, vendor=None):
    frappe.throw("ALD processing not implemented yet")