...
import os
import mimetypes
from alms_app.utils.notification import create_notification_log
...
def _get_email_account_settings(email_id="noreply@merillife.com"):
    try:
        ...
        settings = {
            ...
            'sender_name': getattr(email_account, 'sender_name', 'CRMS'),
            'always_bcc': (
                frappe.conf.get("bcc_email") 
                or getattr(email_account, 'always_bcc', None)
            )
        }
...
def _send_email_with_cc_bcc_attachments(subject, body, to_emails, cc_emails=None, bcc_emails=None, attachments=None):
    ...
    if isinstance(site_config_bcc, str):
        site_config_bcc = [site_config_bcc]

        # Merge both sources uniquely
        bcc_emails = list(set(bcc_emails + site_config_bcc))
