	...
	for lease in leases:
		...
		if lease_doc.status == "Discarded":
			if lease_doc.modifications:
				discarded_on = frappe.db.get_value(
					"Lease Management", lease_doc.modifications[0].modified_lease, "agreement_start_date"
				)
				temp = discarded_on

		report_data = run("Lease Report", filters={"docname": lease.name, "sum_modified": temp})
		...
		if lease_doc.invoice_details and len(lease_doc.invoice_details) > 0:
			for child in lease_doc.invoice_details:
				mdata = month_map.get((child.from_date).strftime("%Y-%m"))
				if mdata:
					# frappe.msgprint(str(mdata)+" "+str(mdata['mlp'])+" "+str(mdata['month_start_date'])+" "+str(mdata['month_end_date']))
					if (
						mdata["month_start_date"] != child.from_date
						or mdata["month_end_date"] != child.to_date
						or mdata["mlp"] != child.amount
					):
						data.append(
							{
								"lease": lease.name,
